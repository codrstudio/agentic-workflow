...
```
Se existir: leia o artefato e o `.env` atual do projeto. Decida como aplicar — sem sobrescrever destrutivamente.

**`brand/`**
```bash
ls {project}/artifacts/brand/ 2>/dev/null
```
Se existir, copie os assets de brand para o local indicado pelo guia de derivacao e execute o comando de derivação `npm run brand:derive`.
